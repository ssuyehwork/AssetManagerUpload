...
class MetadataPanel(QWidget):
    ...
    def show_tag_popup(self):
        ...
        # 【最终修复】创建弹窗时，明确指定使用 'single' 模式
        self.popup = TagSelectionPopup(self.current_tags, self, selection_mode='single')
        self.popup.sig_tag_selected_for_input.connect(self.handle_tag_selected_for_input)
    ...
    def handle_tag_selected_for_input(self, tag_text):
        """【正确实现】当用户从弹窗中选择一个标签时，执行此操作。"""
        # 1. 将标签文本填入输入框
        self.txt_tag_input.setText(tag_text)
        # 2. 让输入框获得焦点
        self.txt_tag_input.setFocus()
        # 3. 关闭弹窗
        if self.popup:
            self.popup.close()
